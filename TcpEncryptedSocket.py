...
class EncSocket(socket.socket):
    ...
    def __init__(self,addr,client=True,protocol=dh_aes) -> None:
        super().__init__(family=socket.AF_INET, type=socket.SOCK_STREAM, proto=0, fileno=None)

        self.client = client
        self.addr = addr

        if protocol != self.dh_aes and protocol != self.rsa:
        ...
        if not client:
            self.public_codes = {}
            self.prime = 16022236204009818131831320183
    ...
    def recieve(self,sock):
        ...
        if prot == self.dh_aes:
           return self.__recieve_dh_aes(sock)
        elif prot == self.rsa:
            return self.__recieve_msg_rsa(sock)
    ...
    def __recieve_dh_aes(self,sock: socket.socket):
        ...
        data = data.split("#@@!?#%@")  # cleaning the data
        data = data[:-1]
    ...
    def __send_pickle_dh_aes(self,sock=None,obj=None):
        if self.prot == self.dh_aes:
            if self.client:
                aes = AESCipher(self.public_key)
            else:
